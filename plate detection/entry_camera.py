import cv2
import subprocess
import time
from picamera2 import Picamera2
from pymongo import MongoClient
from datetime import datetime

# Initialize PiCamera
picam2 = Picamera2()
picam2.preview_configuration.main.size = (640, 480)
picam2.preview_configuration.main.format = "RGB888"
picam2.configure("preview")
picam2.start()

# Initialize MongoDB
MONGO_URI = "------------------------------------" // replace with your actual MongoDB python connection string
client = MongoClient(MONGO_URI)
db = client['parkingDB']
plates_collection = db['plates']

print("🚗 Starting license plate detection and MongoDB storage...")

try:
    while True:
        # Capture frame
        frame = picam2.capture_array()
        img_path = "/tmp/frame.jpg"
        cv2.imwrite(img_path, frame)

        # Run OpenALPR
        result = subprocess.run(
            ['alpr', '-c', 'us', img_path],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE
        )
        output = result.stdout.decode().strip()

        if "plate" in output.lower():
            print("\n✅ Plate detected:")
            print(output)

            # Extract plate number from output
            lines = output.splitlines()
            if len(lines) >= 2:
                parts = lines[1].split()
                plate_number = parts[1] if len(parts) >= 2 else None
                confidence = float(parts[3]) if len(parts) >= 4 else None

                if plate_number:
                    # Check if already in garage
                    existing_plate = plates_collection.find_one({
                        "plate_number": plate_number,
                        "status": "in_garage"
                    })

                    if existing_plate:
                        print(f"ℹ️ Plate {plate_number} is already in garage — no new entry added.")
                    else:
                        plates_collection.insert_one({
                            "plate_number": plate_number,
                            "timestamp_entry": datetime.utcnow(),
                            "timestamp_exit": None,
                            "confidence": confidence,
                            "status": "in_garage"
                        })
                        print(f"📌 Stored plate {plate_number} in MongoDB.")
                else:
                    print("⚠️ Could not parse plate number.")
            else:
                print("⚠️ Unexpected ALPR output format.")

        else:
            print("❌ No license plates found.")

        time.sleep(1)

except KeyboardInterrupt:
    print("\n🛑 Stopped by user.")
